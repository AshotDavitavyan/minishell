...
/*   Updated: 2023/07/28 15:20:47 by vgribkov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_out_outout(t_token *token)
{
	if (token -> redir_flag_out == 1)
		dup2(open(token->redir_fd_out[ft_strlen_2d_arr(token
					->redir_fd_out) - 1], O_RDWR
...
void	redirector(t_token *token)
{
	...
	if (token -> redir_flag_in == 1)
		dup2(open_0(token -> redir_fd_in[ft_strlen_2d_arr(token
					-> redir_fd_in) - 1]), STDIN_FILENO);
	if (token -> redir_flag_out == 1 || token -> redir_flag_outout == 1)
	{
		if (token -> redir_flag_out == 1)
		{
			while (token -> redir_fd_out[++i + 1])
				open(token -> redir_fd_out[i],
