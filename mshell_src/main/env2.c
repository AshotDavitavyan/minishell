...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/28 13:53:45 by adavitav          #+#    #+#             */
/*   Updated: 2023/07/28 13:53:46 by adavitav         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	ft_strlcat_dollar(char **name_new, const char *src, \
size_t destsize, char **name_ptr)
{
	size_t	destlen;
	size_t	srclen;
	char	*dest;
...
int	alloc_vars(char **name, char **var_arr, char **new_name, char *sep)
{
	...
	while (*var_arr)
	{
		...
		if (comp_vars_util(name, var_arr, i, j) == 1)
			return (ft_strlcat_dollar(new_name, move_env(*var_arr), \
			ft_strlen(move_env(*var_arr)) + 1, name));
		var_arr++;
	}
	move_ptr(name);
	return (0);
...
void	alloc_quote_dollar(char **new_name, char **name_ptr, \
char type, char **var_arr)
{
	type = *(*name_ptr)++;
	if (type == 39)
	{
		while (**name_ptr != type && **name_ptr != '\0')
			util_dollar(new_name, name_ptr);
		(*name_ptr)++;
		return ;
	}
	else if (type == 34)
